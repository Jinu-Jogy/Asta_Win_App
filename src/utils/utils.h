...
HANDLE GetMonitor() {
    ...
    HWND windowHandler = GetDesktopWindow() ; // is equal to FindWindow(NULL,NULL) ;

    HMONITOR hMonitor{ MonitorFromWindow(windowHandler, MONITOR_DEFAULTTOPRIMARY) };
    ...
    HANDLE physicalMonitor = PhysicalMonitors->hPhysicalMonitor;
...
COLORREF StringToColor(std::string SourceString) {
	for (int i = 0; i < SourceString.length(); i++) {
        SourceString[i] = tolower(SourceString[i]);
	}
